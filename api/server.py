...
BSCSCAN_API = os.getenv("BSCSCAN_API", "https://api.bscscan.com/api")
...
def compact_score_response(record: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    # A stored collector label is kept, except that GOOD cannot survive an
    # unfinished contract check. Records written before the check had a
    # name are re-read through the current function table.
    gaps = capability_gaps(record)
    if label == "GOOD" and gaps:
...
def api_portfolio():
    ...
    if not Web3.is_address(address):
        return jsonify({"ok": False, "error": "Invalid BNB Chain wallet address"}), 400
...
def fetch_deepseek_verdict(report: dict[str, Any]) -> str | None:
    ...
    prompt = (
        "Analyze this BNB Chain token security scan. "
    ...
    response = requests.post(
        ...
        json={
            ...
            "messages": [
                {"role": "system", "content": "You are RugBuster's concise BNB Chain token risk analyst."},
...
def fetch_portfolio_tokens(address: str) -> list[dict[str, Any]]:
    if not BSCSCAN_API_KEY:
        raise RuntimeError("Portfolio scan requires BSCSCAN_API_KEY on the backend")
    ...
    for row in rows:
        ...
        if token_address and token_address.lower() not in seen:
            seen[token_address.lower()] = {
                "address": token_address,
                "contractAddress": token_address,
                "name": row.get("tokenName") or "Unknown",
                "symbol": row.get("tokenSymbol") or "",
                "balance": row.get("value") or "0",
            }
...
def load_factory_map() -> dict[str, str]:
    network = resolve_network()
    defaults = BSC_TESTNET_FACTORIES if network == "bsc_testnet" else MAINNET_FACTORIES
